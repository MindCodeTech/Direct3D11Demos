...
//--------------------------------------------------------------------------------------
// Default update progress callback (if nullptr is passed to DoUpdateForDirect3D11
//--------------------------------------------------------------------------------------
...
public:
    ...
    {
        IInstallationProgress* prg = nullptr;
        if ( args && SUCCEEDED( args->get_Progress( &prg ) ) )
        {
            long percent = 0;
            prg->get_PercentComplete( &percent );
                              ...
                              void *pContext, UINT *pResult )
{
    ...
    IUpdateSession* pWUSession = nullptr;
    hr = CoCreateInstance( CLSID_UpdateSession, nullptr, CLSCTX_INPROC_SERVER, IID_IUpdateSession, (void**)&pWUSession );
    if ( FAILED(hr) )
    {
    ...
    IUpdateCollection* pWUColl = nullptr;
    hr = CoCreateInstance( CLSID_UpdateCollection, nullptr, CLSCTX_INPROC_SERVER, IID_IUpdateCollection, (void**)&pWUColl );
    if ( FAILED(hr) )
    {
    ...
    IUpdateSearcher* pWUSearcher = nullptr;
    ...
    if ( SUCCEEDED(hr) )
    {
        ...
        ISearchResult* pWUResult = nullptr;
        ISearchJob* pJob = nullptr;
        VARIANT pVar = { 0 };
        ...
        if ( SUCCEEDED(hr) )
        {
            ...
            {
                IUpdateCollection* pWUUpdates = nullptr;
    ...
    if ( update_found )
    {
        IUpdateDownloader* pWUDownloader = nullptr;
        ...
        if ( SUCCEEDED(hr) )
        {
            ...
            if ( SUCCEEDED(hr) )
            {
                IDownloadResult *pWUResult = nullptr;
                IDownloadJob* pJob = nullptr;
                VARIANT pVar = { 0 };
    ...
    {
        IUpdateInstaller* pWUInstaller = nullptr;
        ...
        if ( SUCCEEDED(hr) )
        {
            ...
            if ( SUCCEEDED(hr) )
            {
                IUpdateInstaller2* pWUInstaller2 = nullptr;
                ...
                IInstallationResult *pWUResult = nullptr;
                IInstallationJob* pJob = nullptr;
                VARIANT pVar = { 0 };
...
{
    UINT result;
    HRESULT hr = DoUpdateForDirect3D11( bQuiet ? D3D11IH_QUIET : 0, nullptr, nullptr, &result );
    if ( SUCCEEDED( hr ) )
    {
...
UINT WINAPI SetD3D11InstallMSIProperties( MSIHANDLE hModule )
{
    ...
    if ( szRelativePathToD3D11IH != nullptr )
    {
    ...
    MessageBox( nullptr, sz, L"SetD3D11InstallMSIProperties", MB_OK );
#endif
...
UINT WINAPI DoD3D11InstallUsingMSI( MSIHANDLE hModule )
{
    ...
    if( szCustomActionData )
    {
        ...
        swprintf_s( sz, 1024, L"szCustomActionData=%s\n",
                    szCustomActionData );
        MessageBox( nullptr, sz, L"DoD3D11InstallUsingMSI", MB_OK );
#endif
...
UINT WINAPI FinishD3D11InstallUsingMSI( MSIHANDLE hModule )
{
    ...
    {
#ifdef SHOW_DEBUG_MSGBOXES
        MessageBox( nullptr, L"Reboot is required", L"FinishD3D11InstallUsingMSI", MB_OK );
#endif
    ...
    { 
        MessageBox( nullptr, L"Reboot is NOT required", L"FinishD3D11InstallUsingMSI", MB_OK );
    }
#endif
    return ERROR_SUCCESS;
}
