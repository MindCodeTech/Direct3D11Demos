...
int PASCAL WinMain( _In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR strCmdLine, _In_ int nCmdShow )
{
    ...
    if( !settings.bUninstall && !settings.bUpdate )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            if( !settings.bSilent )
                MessageBox( nullptr, szMsg, TEXT( "GameExplorerInstall" ), MB_OK | MB_ICONINFORMATION );
            bFailure = true;
        }
        else 
        {
    ...
    else if ( settings.bUpdate )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            if( !settings.bSilent )
                MessageBox( nullptr, szMsg, TEXT( "GameExplorerUpdate" ), MB_OK | MB_ICONINFORMATION );
            bFailure = true;
        }
        else 
        {
    ...
    else if ( settings.bUninstall )
    {
        ...
        if( FAILED( hr ) )
        {
            ...
            if( !settings.bSilent )
                MessageBox( nullptr, szMsg, TEXT( "GameExplorerUninstall" ), MB_OK | MB_ICONINFORMATION );
            bFailure = true;
        }
        else 
        {
                    ...
                    for(; ; )
                    {
                        ...
                        if( SUCCEEDED( hr ) && uReturned != 0 && pGameClass != nullptr )
                        {
                            ...
                            swprintf_s( szMsg, 256, L"Remove %s [%s] [%s]?", strGameName, strGDFBinaryPath,
                                             strGameGUID );
                            if( IDYES == MessageBox( nullptr, szMsg, L"GDFInstall", MB_YESNO ) )
                            {
        ...
        if( *strCmdLine == L'/' || *strCmdLine == L'-' )
        {
            ...
            if( IsNextArg( strCmdLine, L"installpath" ) )
            {
                ...
                if( !pSettings->bSilent )
                    MessageBox( nullptr, L"Incorrect flag usage: /installpath\n", L"GDFInstall", MB_OK );
                continue;
            }
