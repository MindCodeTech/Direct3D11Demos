...
UINT __stdcall GameExplorerSetMSIProperties(MSIHANDLE hModule)
{
    ...
    if (szInstallDir != nullptr)
    {
        ...
        if (szRelativePathToGDF != nullptr )
        {
...
UINT __stdcall GameExplorerInstallUsingMSI(MSIHANDLE hModule)
{
    ...
    if (szCustomActionData != nullptr)
    {
        ...
        MessageBox(nullptr, sz, L"GameExplorerInstallUsingMSI", MB_OK);
#else
...
UINT __stdcall GameExplorerUninstallUsingMSI(MSIHANDLE hModule)
{
    ...
    if (szCustomActionData)
    {
        ...
        MessageBox(nullptr, sz, L"GameExplorerUninstallUsingMSI", MB_OK);
#else
                            ...
                            GAME_INSTALL_SCOPE InstallScope)
{
    ...
    if (strGDFBinPath == nullptr || strGameInstallPath == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            if (fIsV2GDF)
            {
                IGameExplorer2* pFwGameExplorer2 = nullptr;
                // Create an instance of the IGameExplorer2 Interface
                hr = CoCreateInstance(__uuidof(GameExplorer), nullptr, CLSCTX_INPROC_SERVER, __uuidof(IGameExplorer2), (void**)&pFwGameExplorer2);
                if (SUCCEEDED(hr) && (pFwGameExplorer2 != nullptr))
                {
                    // Windows 7
                ...
                {
                    IGameExplorer* pFwGameExplorer = nullptr;
                    // Create an instance of the IGameExplorer Interface
                    hr = CoCreateInstance(__uuidof(GameExplorer), nullptr, CLSCTX_INPROC_SERVER, __uuidof(IGameExplorer), (void**)&pFwGameExplorer);
                    if (SUCCEEDED(hr) && (pFwGameExplorer != nullptr))
                    {
                        // Windows Vista
                                 ...
                                 GAME_INSTALL_SCOPE InstallScope)
{
    ...
    HRESULT hr = E_FAIL;
    BSTR bstrGDFBinPath = nullptr;
    BSTR bstrGameInstallPath = nullptr;
    
    bstrGDFBinPath = SysAllocString(strGDFBinPath);
    bstrGameInstallPath = SysAllocString(strGameInstallPath);
    if (bstrGDFBinPath != nullptr && bstrGameInstallPath != nullptr)
    {
        ...
        //
        HKEY hKeyGamesToFind = nullptr, hKeyGame = nullptr;
        LONG lResult;
        DWORD dwDisposition;
        if (InstallScope == GIS_CURRENT_USER)
        {
            lResult = RegCreateKeyEx(HKEY_CURRENT_USER,
                                      ...
                                      0, nullptr, 0, KEY_WRITE, nullptr, &hKeyGamesToFind, &dwDisposition);
        }
        else
        {
        ...
        MessageBox(nullptr, sz, L"GameExplorerInstallForXP", MB_OK);
#endif
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                lResult = RegCreateKeyEx(hKeyGamesToFind, strGameInstanceGUID, 0, nullptr, 0, KEY_WRITE,
                                          nullptr, &hKeyGame, &dwDisposition);
                hr = HRESULT_FROM_WIN32(lResult);
                if (SUCCEEDED(hr))
                {
                                    ...
                                    GAME_INSTALL_SCOPE InstallScope)
{
    ...
    if (strGDFBinPath == nullptr || strGameInstallPath == nullptr || pFwGameExplorer == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if ((bstrGDFBinPath != nullptr) && (bstrGameInstallPath != nullptr))
    {
        ...
        MessageBox(nullptr, sz, L"GameExplorerInstallUsingIGameExplorer", MB_OK);
#endif
                                   ...
                                   GAME_INSTALL_SCOPE InstallScope)
{
    ...
    if (strGDFBinPath == nullptr || strGameInstallPath == nullptr || pFwGameExplorer2 == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (bstrGDFBinPath != nullptr && bstrGameInstallPath != nullptr)
    {
       ...
       swprintf_s(sz, 1024, L"szGDFBinPath='%s'\nszGameInstallPath='%s'\nInstallScope='%s'\nszGUID='%s'\nAccount=%s\\%s\nAdmin=%d\nhr=0x%0.8x",
                        bstrGDFBinPath, bstrGameInstallPath, (InstallScope == GIS_ALL_USERS) ? L"GIS_ALL_USERS" : L"GIS_CURRENT_USER", strGameInstanceGUID, strDomain, strUser, bAdmin, hr);
       MessageBox(nullptr, sz, L"GameExplorerInstallUsingIGameExplorer2", MB_OK);
#endif
...
HRESULT GameExplorerUninstallFromRegistry(WCHAR* strGDFBinPath)
{
    ...
    MessageBox(nullptr, sz, L"GameExplorerUninstallForXP", MB_OK);
#endif
...
HRESULT GameExplorerUninstallUsingIGameExplorer(IGameExplorer* pFwGameExplorer, WCHAR* strGDFBinPath)
{
    ...
    if (strGDFBinPath == nullptr || pFwGameExplorer == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    MessageBox(nullptr, sz, L"GameExplorerUninstallUsingIGameExplorer", MB_OK);
#endif
...
HRESULT GameExplorerUninstallUsingIGameExplorer2(IGameExplorer2* pFwGameExplorer2, WCHAR* strGDFBinPath)
{
    ...
    if (strGDFBinPath == nullptr || pFwGameExplorer2 == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    WCHAR sz[1024];
    swprintf_s(sz, 1024, L"\npInstanceGUID='%s'\nhr=0x%0.8x", strGameInstanceGUID, hr);
    MessageBox(nullptr, sz, L"GameExplorerUninstallUsingIGameExplorer2", MB_OK);
#endif
...
HRESULT GameExplorerUpdateUsingIGameExplorer(IGameExplorer* pFwGameExplorer, WCHAR* strGDFBinPath)
{
    ...
    StringFromGUID2(InstanceGUID, strGameInstanceGUID, ARRAYSIZE(strGameInstanceGUID));
    swprintf_s(sz, 1024, L"\npInstanceGUID='%s'\nhr=0x%0.8x", strGameInstanceGUID, hr);
    MessageBox(nullptr, sz, L"GameExplorerUpdateUsingIGameExplorer", MB_OK);
#endif
...
HRESULT ConvertStringToGUID(const WCHAR* strSrc, GUID* pGuidDest)
{
    ...
    if (strSrc == nullptr || pGuidDest == nullptr)
    {
        return E_INVALIDARG;
    }
                               ...
                               int cchDestChar)
{
    ...
    if (pGuidSrc == nullptr || strDest == nullptr)
    {
        return E_INVALIDARG;
    }
                                       ...
                                       GUID* pGUID)
{
    ...
    for(; ;)
    {
        ...
        lResult = RegCreateKeyEx(hKeyRoot, strRegKeyName, 0, nullptr, 0, KEY_READ, nullptr, &hKey, &dwDisposition);
        if (lResult == ERROR_SUCCESS)
        {
...
HRESULT RetrieveGUIDForApplication(WCHAR* szPathToGDFdll, GUID* pGUID)
{
    ...
    if (szPathToGDFdll == nullptr || pGUID == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr) && pIWbemLocator)
        {
            ...
            if (SUCCEEDED(hr) && (pIWbemServices != nullptr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    IWbemClassObject* pGameClass = nullptr;
                    DWORD uReturned = 0;
                    BSTR pPropName = nullptr;

                    // Get the first one in the list
                    hr = pEnum->Next(5000, 1, &pGameClass, &uReturned);
                    if (SUCCEEDED(hr) && (uReturned != 0) && (pGameClass != nullptr))
                    {
                        VARIANT var;

                        // Get the InstanceID string
                        pPropName = SysAllocString(L"InstanceID");
                        hr = pGameClass->Get(pPropName, 0L, &var, nullptr, nullptr);
    ...
    MessageBox(nullptr, sz, L"RetrieveGUIDForApplication", MB_OK);
#endif
    ...
    if (!bFound)
    {
        ...
        lResult = RegCreateKeyEx(HKEY_CURRENT_USER,
                                  L"Software\\Classes\\Software\\Microsoft\\Windows\\CurrentVersion\\GameUX\\GamesToFindOnWindowsUpgrade",
                                  0, nullptr, 0, KEY_READ, nullptr, &hKeyRoot, &dwDisposition);
                       ...
                       DWORD cchDomain)
{
    ...
    if (strUser == nullptr || strDomain == nullptr )
    {
        return E_INVALIDARG;
    }
    ...
    if (WTSEnumerateProcesses(hServer, 0, 1, &pProcessInfo, &dwCount))
    {
        for(DWORD n = 0; n < dwCount; n++)
        {
            ...
            {
                SID_NAME_USE eUse;
                BOOL bSuccess = LookupAccountSid(nullptr, pProcessInfo[n].pUserSid, strUser, &cchUser,
                        ...
                        int cchValue)
{
    ...
    if (pNode == nullptr || strAttribName == nullptr || strValue == nullptr)
    {
        return E_INVALIDARG;
    }
...
HRESULT GetBaseKnownFolderCsidl(WCHAR* strBaseKnownFolder, int* pCsidl)
{
    ...
    if (strBaseKnownFolder == nullptr || pCsidl == nullptr )
    {
        return E_INVALIDARG;
    }
...
//-----------------------------------------------------------------------------
HRESULT CreateShortcut(WCHAR* strLaunchPath, 
                       WCHAR* strCommandLineArgs,      // strCommandLineArgs can be nullptr
                       WCHAR* strShortcutFilePath, 
                       BOOL bFileTask)
{
    ...
    if (strLaunchPath == nullptr || strShortcutFilePath == nullptr )
    {
        return E_INVALIDARG;
    }
...
HRESULT SubCreateSingleTask(GAME_INSTALL_SCOPE InstallScope,         // Either GIS_CURRENT_USER or GIS_ALL_USERS 
                   ...
                   WCHAR* strCommandLineArgs,               // Can be nullptr.  Example: "-windowed"
                   ...
                   BOOL bFileTask)                          // if TRUE, this is a file task otherwise it is a URL task
{
    ...
    if (strGDFBinPath == nullptr || strTaskName == nullptr || strLaunchPath == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (InstallScope == GIS_CURRENT_USER)
    {
        SHGetFolderPath(nullptr, CSIDL_LOCAL_APPDATA, nullptr, SHGFP_TYPE_CURRENT, strCommonFolder);
    }
    else
    {
        SHGetFolderPath(nullptr, CSIDL_COMMON_APPDATA, nullptr, SHGFP_TYPE_CURRENT, strCommonFolder);
    }
    ...
    SHCreateDirectoryEx(nullptr, strPath, nullptr);
    ...
    MessageBox(nullptr, sz, L"CreateTask", MB_OK);
#endif
...
HRESULT RemoveTasks(WCHAR* strGDFBinPath) // valid GameInstance GUID that was passed to AddGame()
{
    ...
    // Get base path based on install scope
    if (FAILED(hr = SHGetFolderPath(nullptr, CSIDL_LOCAL_APPDATA, nullptr, SHGFP_TYPE_CURRENT, strLocalAppData)))
    {
        return hr;
    }
    if (FAILED(hr = SHGetFolderPath(nullptr, CSIDL_COMMON_APPDATA, nullptr, SHGFP_TYPE_CURRENT, strCommonAppData)))
    {
        return hr;
    }
                           ...
                           BOOL bSupportTask)
{
    ...
    if (pTaskNode == nullptr || strGDFBinPath == nullptr || strGameInstallPath == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            IXMLDOMNode* pFileTaskNode = nullptr;
            pTaskNode->selectSingleNode(L"FileTask", &pFileTaskNode);
            if (pFileTaskNode != nullptr)
            {
                WCHAR strPath[256] = {0};
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (hr == S_OK)
                        {
                            ...
                            if (hr == S_OK)
                            {
                                ...
                                hr = SHGetFolderPath(nullptr, nCsidl, nullptr, SHGFP_TYPE_CURRENT, strFolderPath);
                                if (SUCCEEDED(hr))
                                {
            ...
            {
                IXMLDOMNode* pURLTaskNode = nullptr;
                pTaskNode->selectSingleNode(L"URLTask", &pURLTaskNode);
                if (pURLTaskNode != nullptr)
                {
                    WCHAR strURLPath[256] = {0};
                    hr = GetXMLAttribute(pURLTaskNode, L"Link", strURLPath, 256);
                    if (SUCCEEDED(hr))
                    {
                        hr = SubCreateSingleTask(InstallScope, strGDFBinPath, strTaskName, strURLPath, nullptr, nTaskID, bSupportTask, false);
                    }
                }
            }
        }
    ...
    if( SUCCEEDED( hr ) )
    {
        ...
        if (pRootNode != nullptr)
        {
            ...
            if (pPlayTasksNode != nullptr)
            {
                ...
                if (pPrimaryPlayTaskNode != nullptr)
                {
                    ...
                    if( SUCCEEDED( hr ) )
                    {
                        ...
                        while (pSecondaryPlayTaskNode != nullptr)
                        {
            ...
            else
            {
                ...
                MessageBox(nullptr, sz, L"GameExplorerInstall", MB_OK);
#endif
                hr = S_FALSE;
            }
        }
    ...
    )
{
    ...
    assert(pfV2GDF);
        ...
        MessageBox(nullptr, sz, L"IsV2GDF", MB_OK);
#endif

    if (strGDFBinPath == nullptr || pfV2GDF == nullptr)
    {
        return E_INVALIDARG;
    }
    ...
    *pfV2GDF = FALSE;
    ...
    if( SUCCEEDED( hr ) )
    {
        ...
        if (pRootNode != nullptr)
        {
            IXMLDOMNode* pPrimaryPlayTasksNode = nullptr;
