...
HRESULT InitAudio()
{
	...
	g_audioState.vListenerPos = XMFLOAT3(0, 0, 0);
	g_audioState.vEmitterPos = XMFLOAT3(0, 0, float(ZMAX));
	...
	g_audioState.listener.OrientFront = XMFLOAT3(0, 0, 1);
	g_audioState.listener.OrientTop = XMFLOAT3(0, 1, 0);
	...
	g_audioState.emitter.OrientFront = XMFLOAT3(0, 0, 1);
	g_audioState.emitter.OrientTop = XMFLOAT3(0, 1, 0);
...
HRESULT UpdateAudio( float fElapsedTime )
{
    ...
    if( g_audioState.nFrameToApply3DAudio == 0 )
    {
        ...
        {
            XMFLOAT3 vDelta = g_audioState.vListenerPos - g_audioState.listener.Position;

            g_audioState.fListenerAngle = float( atan2( vDelta.x, vDelta.z ) );

            vDelta.y = 0.0f;
            XMVector3Normalize( &vDelta );
        ...
        if( fElapsedTime > 0 )
        {
            XMFLOAT3 lVelocity = ( g_audioState.vListenerPos - g_audioState.listener.Position ) / fElapsedTime;
            g_audioState.listener.Position = g_audioState.vListenerPos;
            g_audioState.listener.Velocity = lVelocity;

			XMFLOAT3 eVelocity = (g_audioState.vEmitterPos - g_audioState.emitter.Position) / fElapsedTime;
            g_audioState.emitter.Position = g_audioState.vEmitterPos;
