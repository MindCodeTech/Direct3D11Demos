                              ...
                              )  
{
    ...
    if ( !m_pvsRenderOrthoShadowBlob ) 
    {
        ...
        L"\\..\\Media\\Shaders\\RenderCascadeShadow.hlsl", nullptr, "VSMain", m_cvsModel, D3DCOMPILE_ENABLE_STRICTNESS, 0, &m_pvsRenderOrthoShadowBlob ) );
    }
    ...
    for( INT iCascadeIndex=0; iCascadeIndex < MAX_CASCADES; ++iCascadeIndex ) 
    {
        ...
        {
            V_RETURN( DXUTCompileFromFile( L"\\..\\Media\\Shaders\\RenderCascadeScene.hlsl", defines, "VSMain", 
        ...
        for( INT iDerivativeIndex=0; iDerivativeIndex < 2; ++iDerivativeIndex ) 
        {
            for( INT iBlendIndex=0; iBlendIndex < 2; ++iBlendIndex ) 
            {
                for( INT iIntervalIndex=0; iIntervalIndex < 2; ++iIntervalIndex ) 
                {
                    ...
                    {
                        V_RETURN( DXUTCompileFromFile( L"\\..\\Media\\Shaders\\RenderCascadeScene.hlsl", defines, "PSMain", 
